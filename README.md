...
* **ICE / STUN Support**: `callrs` now supports NAT traversal using ICE (Interactive Connectivity Establishment) and STUN (Session Traversal Utilities for NAT). 
* **Automatic Candidate Gathering**: When hosting, the application gathers local and public ICE candidates automatically using the STUN server `stun.l.google.com:19302`.
* **P2P Connection Across NATs**: Clients can connect using a session offer generated by the host, allowing peer-to-peer audio calls even behind NAT or firewalls.
* **Session Offer**: Copy the base64-encoded session data from the host and paste it into the Connect mode to initiate the call.
